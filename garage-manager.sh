...
# Execute garage commands with proper error handling
garage_exec() {
    local cmd="$1"
    local full_cmd="/usr/local/bin/garage $cmd"
    
    log "Executing: garage $cmd"
    if ! sudo docker exec ${CONTAINER_NAME} sh -c "$full_cmd" 2>/dev/null; then
        ...
        for path in "${alt_paths[@]}"; do
            warn "Trying alternative path: $path"
            if sudo docker exec ${CONTAINER_NAME} sh -c "$path $cmd" 2>/dev/null; then
                return 0
            fi
        done
        
        error "Failed to execute garage command. Garage binary not found in container."
        error "This might be due to:"
        echo "  - Wrong Docker image"
        echo "  - Container not properly initialized"
        echo "  - Binary not in expected location"
        return 1
    fi
    return 0
}
...
setup_garage() {
    ...
    echo "1. dxflrs/garage:v2.0.0 (default)"
    echo "2. dxflrs/garage:latest" 
    echo "3. Custom image"
    echo ""
    ...
    if sudo docker run \
        ...
        error "Failed to start container. Check if ports are already in use."
    fi
    pause
}
...
configure_cluster() {
    ...
    if ! NODE_ID=$(garage_exec "node id" | head -n1); then
        error "Failed to get node ID - garage binary may not be available"
        return 1
    fi
...
change_ports() {
    ...
    # Get current image
    local current_image=$(sudo docker inspect ${CONTAINER_NAME} --format='{{.Config.Image}}' 2>/dev/null || echo "dxflrs/garage:v2.0.0")
...
troubleshoot() {
    ...
    if container_running; then
        echo ""
        echo -e "${CYAN}3. Garage Binary Locations in Container:${NC}"
        local binary_paths=(
        ...
        echo ""
        echo -e "${CYAN}4. Container Process List:${NC}"
        sudo docker exec ${CONTAINER_NAME} ps aux 2>/dev/null || echo "Unable to get process list"
        
        echo ""
        echo -e "${CYAN}5. Environment Variables:${NC}"
        sudo docker exec ${CONTAINER_NAME} env | grep -i path 2>/dev/null || echo "Unable to get environment"
    fi
